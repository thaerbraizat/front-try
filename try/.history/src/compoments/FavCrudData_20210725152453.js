...
export class FavCrudData extends Component {
    ...
    updateForm = (e) => {
        ...
        const object = {
            ...
            thumbnail: e.target.elements.des1.value,

        }
        axios.put(`http://localhost:8001/fav/${this.state.item.title}`, object).then(res => {
            this.setState({
                item: { ...this.state.item, artist_display: e.target.elements.des.value, thumbnail: e.target.elements.des1.value },
                showForm: false
            })
        })
