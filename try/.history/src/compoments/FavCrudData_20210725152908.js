...
export class FavCrudData extends Component {
    ...
    updateForm = (e) => {
        e.preventDefault();
        const object = {
            artist_display: e.target.elements.des.value,
            attack_type: e.target.elements.des1.value,
            img: e.target.elements.des1.value,

        }
        axios.put(`http://localhost:8001/fav/${this.state.item.localized_name}`, object).then(res => {
            this.setState({
                item: { ...this.state.item, localized_name: e.target.elements.des.value, localized_name: e.target.elements.des1.value },
                showForm: false
            })
        })
